...
export default function DaYunFlow({
  ...
  const fortuneColorDark = (level: string) => {
    switch (level) {
      case '大吉':
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ===== 顶部当前定位卡（人话版为主） ===== */}
      {currentDaYun && (
        <div className="bg-gradient-to-br from-[#2a1f3d] to-[#1a1330] text-white rounded-xl p-5 shadow-lg">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            ...
            <div className="flex flex-col items-center sm:items-end gap-2 sm:ml-4">
              <div
                className={`inline-flex items-center justify-center w-14 h-14 rounded-full text-lg font-bold ${fortuneColorDark(
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-white/70">运势评分</span>
              </div>
            </div>
          </div>
      ...
      <div className="moonly-card p-4">
        ...
        {/* 图例 */}
        <div className="flex items-center gap-4 mt-3 text-xs text-moonly-text-muted">
          <div className="flex items-center gap-1">
      ...
      {/* ===== 大运详情面板 - 增加头部标签栏 ===== */}
      {selectedDaYun && (
        <div ref={detailRef} className="moonly-card">
          {/* 顶部标签栏 */}
          <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-white/10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-moonly-text-muted">
                {selectedDaYun.startYear}-{selectedDaYun.endYear}
              </span>
            </div>
            <div className="flex items-center gap-2 bg-white/5 rounded-full p-1">              <button
                onClick={() => setViewMode('simple')}
                className={`text-xs px-3 py-1 rounded-full transition-all ${
                  viewMode === 'simple'
                    ? 'bg-white/10 text-gold shadow-sm font-bold'
                    : 'text-moonly-text-muted hover:text-white/70'
                }`}
              >
                👤 简易
              </button>
          ...
          <div className="flex items-center justify-between mb-4 pb-4 border-b border-white/10">
            ...
            <div className="text-right flex flex-col items-end gap-1">
              <div
                className={`inline-block px-4 py-2 rounded-full text-lg font-bold ${fortuneColorDark(
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-moonly-text-muted">运势评分</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-lg p-4 mb-4">
            ...
            {viewMode === 'simple' && (
              <p className="text-xs text-moonly-text-secondary mt-2">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              {(() => {
                ...
                return items.map((item) => (
                  <div
                    key={item.key}
                    className={`moonly-card-light p-3 shadow-sm ${item.color}`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-lg">{item.icon}</span>
                      <span className="text-xs font-bold text-moonly-text-secondary">{item.title}</span>
                    </div>
          ...
          {viewMode === 'pro' && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div className="bg-white/5 border border-white/10 rounded-lg p-3 text-center shadow-sm">
                <div className="text-xs text-moonly-text-muted mb-1">大运天干</div>
                <div className="text-2xl font-bold text-gold">{selectedDaYun.gan}</div>
                <div className="text-xs text-moonly-text-muted mt-1">{selectedDaYun.yinYang} · {selectedDaYun.wuXing}</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-lg p-3 text-center shadow-sm">
                <div className="text-xs text-moonly-text-muted mb-1">十神关系</div>
                <div className="text-lg font-bold text-gold">{selectedDaYun.shiShen}</div>
                <div className="text-xs text-moonly-text-muted mt-1">对日主{dayMaster}</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-lg p-3 text-center shadow-sm">
                <div className="text-xs text-moonly-text-muted mb-1">地支</div>
                <div className="text-2xl font-bold text-gold">{selectedDaYun.zhi}</div>
                <div className="text-xs text-moonly-text-muted mt-1">藏干待展开</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-lg p-3 text-center shadow-sm">
                <div className="text-xs text-moonly-text-muted mb-1">运势标签</div>
                <div className="flex flex-wrap gap-1 justify-center mt-1">
                  {selectedDaYun.keywords.slice(0, 2).map((k, i) => (
                    <span
                      key={i}
                      className="text-xs bg-white/10 px-2 py-0.5 rounded text-gold"
                    >
                      {k}
                    </span>
                  ))}
          ...
          <button
            ...
            className="w-full btn-gold py-3 rounded-lg font-medium transition-colors mb-4 disabled:opacity-50"
          >
            {loadingDaYunAi === selectedDaYun.index
              ? 'AI 正在深度解读...'
              : daYunAiResult[selectedDaYun.index]
          ...
          {daYunAiResult[selectedDaYun.index] && (
            <div className="bg-white/5 rounded-lg p-4 mb-4">
              <h4 className="font-bold text-white mb-2">AI 大运深度解读</h4>
              <div className="prose max-w-none text-sm text-white whitespace-pre-line">
                {daYunAiResult[selectedDaYun.index]}
              </div>
            </div>
          )}
          ...
          {/* 流年：默认今年+明年，可展开 */}
          <h4 className="font-bold text-white mb-3">此运流年</h4>
            ...
            {selectedDaYun.years
              ...
              .map((ln) => (
                <div
                  ...
                  className={`cursor-pointer rounded-lg border-2 p-2 text-center transition-all ${
                    selectedLiuNian?.year === ln.year
                      ? 'border-moonly-gold/50 bg-white/5 scale-105'
                      : ln.isCurrent
                ...
                >
                  ...
                  {ln.isCurrent && (
                    <div className="text-xs text-gold mt-0.5">今年</div>
                  )}
                  {ln.year === new Date().getFullYear() + 1 && (
                    <div className="text-xs text-moonly-gold/70 mt-0.5">明年</div>
                  )}
                </div>
              ))}
          </div>
          ...
          <button
            ...
            className="w-full text-center text-sm text-gold hover:text-white py-2 border border-white/10 rounded-lg transition-colors"
          >
            {showAllLiuNian
              ? '收起 · 只看今年明年'
      ...
      {/* ===== 流年详情弹窗 ===== */}
      {selectedLiuNian && (
        <div className="moonly-card p-5 border-2 border-white/10">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="text-moonly-text-muted hover:text-white text-xl"
            >
              ✕
            </button>
          </div>
          ...
          {viewMode === 'simple' && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-3 mb-4">
              ...
              <p className="text-xs text-moonly-text-secondary mt-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <div className="bg-white/5 rounded-lg p-3 text-center">
              <div className="text-xs text-moonly-text-secondary">
            ...
            <div className="bg-white/5 rounded-lg p-3 text-center">
              <div className="text-xs text-moonly-text-secondary">运势等级</div>
              <div
          ...
          {viewMode === 'pro' && (
            <div className="bg-white/5 rounded-lg p-4 mb-4">
              <h4 className="font-bold text-white mb-2">年度主题</h4>
              <p className="text-sm text-moonly-text-secondary">
          ...
          {/* 关键月份提醒 */}
          <div className="mb-4">
            <h4 className="font-bold text-white mb-3">关键月份提醒</h4>
                  ...
                  <span
                    className={`text-sm font-bold ${
                      mh.level === '吉'
                        ...
                        : mh.level === '凶'
                        ? 'text-red-400'
                        : 'text-moonly-text-secondary'
                    }`}
                  >
                    {mh.level === '吉' ? '✓' : mh.level === '凶' ? '⚠' : '—'}
                  </span>
                  <span className="text-sm text-white">{mh.desc}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {liuNianAiResult[selectedLiuNian.year] && (
            <div className="bg-white/5 rounded-lg p-4">
              <h4 className="font-bold text-white mb-2">AI 流年深度解读</h4>
              <div className="prose max-w-none text-sm text-white whitespace-pre-line">
                {liuNianAiResult[selectedLiuNian.year]}
              </div>
            </div>
