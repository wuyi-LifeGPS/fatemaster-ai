...
export function buildDaYunPrompt(
  ...
  try {
    ...
    return [
      ...
      `- 姓名：${name || '未提供'}`,
      `- 性别：${gender === 'male' ? '男' : '女'}`,
      `- 日主：${dayMaster || '未知'}（${bodyStrength?.strength || '未知'}）`,
      `- 格局：${pattern?.patternName || '未知'}`,
    ...
    console.error('buildDaYunPrompt error:', e);
    return '请分析此大运的运势特点和注意事项。';
  }
}
...
export function buildLiuNianPrompt(
  ...
  try {
    ...
    return [
      ...
      ...(liuNian.monthHighlights?.map(mh => `- ${mh.desc}（${mh.level}）`) || []),
      '',
      '七、年度宜忌清单',
      '- 今年最适合做的3件事',
      '- 今年最需要避免的3件事',
    ...
    console.error('buildLiuNianPrompt error:', e);
    return '请分析此流年的运势特点和注意事项。';
  }
}
